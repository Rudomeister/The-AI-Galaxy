...
async def get_orchestrator_ws(websocket: WebSocket) -> SystemOrchestrator:
    """
    Get the system orchestrator instance for WebSocket endpoints.
...
async def get_redis_service_ws(websocket: WebSocket) -> RedisService:
    """
    Get the Redis service instance for WebSocket endpoints.
...
async def get_vector_search_service_ws(websocket: WebSocket) -> VectorSearchService:
    """
    Get the vector search service instance for WebSocket endpoints.
    ...
    Raises:
        HTTPException: If vector search service is not available
    """
    vector_service = getattr(websocket.app.state, 'vector_search_service', None)
    if not vector_service:
        logger.error("Vector search service not available in app state")
        raise HTTPException(
            status_code=503,
