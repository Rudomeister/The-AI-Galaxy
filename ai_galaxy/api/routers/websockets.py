...
from ..dependencies import get_redis_service_ws, get_orchestrator_ws
...
async def websocket_agents_endpoint(
    websocket: WebSocket,
    redis_service: RedisService = Depends(get_redis_service_ws),
    orchestrator: SystemOrchestrator = Depends(get_orchestrator_ws)
):
    """
...
async def websocket_monitor_endpoint(
    ...
    """
    WebSocket endpoint for real-time system monitoring and activity feed.
    
    Provides comprehensive live updates about all system activities including
    agent events, workflow transitions, system metrics, and error monitoring.
    """
    ...
    try:
        ...
        while True:
            try:
                ...
                logger.error(f"Error handling monitor WebSocket message: {str(e)}")
                await manager.send_personal_message(
                    {"type": "error", "message": "Message processing error"},
                    websocket
                )
